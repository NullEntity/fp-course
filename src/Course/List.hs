...
-- Empty
-- I want to aggregate. I want to map each optional value
-- Some 1 :. Some 2 :. Empty :. Nil
-- >> :t foldLeft
-- foldLeft :: (b -> a -> b) -> b -> List a -> b
-- a :: Optional a
-- b :: Optional (List a)

-- mapOptional :: (c -> d) -> Optional c -> Optional d
--
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldLeft (\acc-> \x-> mapOptional (:.)) Nil
